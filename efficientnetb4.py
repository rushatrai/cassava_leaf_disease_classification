...
class EfficientNet(pl.LightningModule):
    def __init__(self, config, version, num_classes):
        super().__init__()
    ...
    def calculate_factors(self, version, alpha=1.2, beta=1.1):  # already have resolution
    ...
    def create_features(self, width_factor, depth_factor, last_channels):
        ...
        for expand_ratio, channels, repeats, stride, kernel_size in base_model:
            # keeps out_channels always divisible by 4 (for downsample in residual block)
            ...
            for layer in range(layers_repeats):
                features.append(
                    InvertedResidualBlock(
                        ...
                        stride=stride if layer == 0 else 1,  # only first layer in block has stride > 1
